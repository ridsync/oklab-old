...
var app = {
    ...
    var socket = io('/chatroom', { transports: ['websocket'] });
      ...
      socket.on('connect', function () {
        ...
        socket.on('updateUsersList', function(users, clear) {
            console.log('updateUsersList call users = ' + JSON.stringify(users));
          $('.container p.message').remove();
          if(users.error != null){
            $('.container').html(`<p class="message error">${users.error}</p>`);
          }else{
            console.log('updateUsersList helpers call !! ' + clear);
  ...
  helpers: {
    ...
    updateRoomsList: function(room){
      ...
      var html = `<a href="/chat/roomId/${room._id}"><li class="room-item">${room.title}</li></a>`;
